...
public class HumanStepEventResponder implements StepEventListener {
	...
	public HumanStepEventResponder(CellGameField [][] cellArray, WinEventProducer winEventProducer,DeadHeatEventProducer deadHeatEventProducer, int numberSymbolsForWin) {
	...
	public void stepEventHappend(StepEvent stepEvent) {
		...
		if(stepEvent.gethumanIdentifer()) { //если ход сделал человек X
			...
			if(NeoLogicGame.checkWin(field, numberSymbolsForWin, cellArray)) {
				...
				JOptionPane.showMessageDialog(null, "Human X Win!!!");
			}
			
			else if (NeoLogicGame.checkDeadHeat(cellArray)) {
		...
		else { //если ход сделал человек O
			...
			if(!field.isGameOverFlag()) {
				...
				if(NeoLogicGame.checkWin(field, numberSymbolsForWin, cellArray)) {
					...
					JOptionPane.showMessageDialog(null, "Human O Win!!!");
				}
